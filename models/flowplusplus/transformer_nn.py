...
class TransformerNN(nn.Module):
    ...
    """
    def __init__(self, in_channels, out_channels, num_channels, num_layers, num_heads, num_components, drop_prob, use_pos_emb):
        #import pdb;pdb.set_trace()
        super(TransformerNN, self).__init__()
        self.k = num_components  # k = number of mixture components
        # import pdb;pdb.set_trace()
        self.transformer = BasicTransformerModel(out_channels * (2 + 3 * self.k), in_channels, num_heads, num_channels, num_layers, drop_prob, use_pos_emb=use_pos_emb)
